...
namespace IntakeConstants {
    ...
    constexpr auto holdExtendRequest = 
        ctre::phoenix6::controls::TorqueCurrentFOC{gravityTorqueCurrent/8}
        .WithIgnoreSoftwareLimits(true)
    ;
    ...
    constexpr auto outakingSurfaceSpeed = -6_mps;
...
frc2::CommandPtr Intake::BlindRetract() {
    return
        Run([this] {m_armMotor.SetControl(IntakeConstants::blindRetractRequest);})
            .WithTimeout(IntakeConstants::retractTime*2)
        .AndThen(RunOnce([this] {HoldRetracted();}))
    ;
}
...
frc2::CommandPtr Intake::HomeArm() {
    ...
    auto hard_stopped = frc2::Trigger{[this] {
        return frc::IsNear(0_tps, m_armMotor.GetVelocity().GetValue(), 0.05_tps)
            && m_armMotor.GetStatorCurrent().GetValue() > 9_A;
    }}.Debounce(0.1_s);
...
frc2::CommandPtr Intake::SeedArm() {
    return SeedArm(IntakeConstants::armInPos);
}
